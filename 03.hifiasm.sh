#!/bin/bash
#PBS -l select=1:ncpus=23:mem=80GB
#PBS -q bix
...
HIFIASM_ASM_l0="${HIFIASM_DIR}/l0/dSAMPLE_CLI.asm"
HIFIASM_ASM_primary="${HIFIASM_DIR}/primary/dSAMPLE_CLI.asm"
...
FASTA_PRIMARY_RENAMED="${HIFIASM_DIR}/dSAMPLE_CLI_primary_renamed.fa"
FASTA_ALTERNATE_RENAMED="${HIFIASM_DIR}/dSAMPLE_CLI_alternate_renamed.fa"

#make temp directory to copy reads to so the original ones are accessible to other scripts
mkdir -p "${TEMP_DIR}" "${HIFIASM_ASM_l0}" "${HIFIASM_ASM_l0}"
cp "${RAW_READS_FQ}" "${TEMP_DIR}/"
RAW_READS_FQ="${TEMP_DIR}/D260405-SAMPLE_CLI_HiFi.fastq.gz"

#hifiasm assembly
echo "Performing contig assembly..."
hifiasm -o "${HIFIASM_ASM_primary}" --primary -t "${THREADS}" "${RAW_READS_FQ}" || { echo "Contig assembly failed"; exit 1; }
hifiasm -o "${HIFIASM_ASM_l0}" -l0 -t "${THREADS}" "${RAW_READS_FQ}" || { echo "Contig assembly failed"; exit 1; }
...
for i in "${!GFA_FILES[@]}"; do
    ...
    sed -E 's/^>(.*)/>dSAMPLE_CLI_\1/' "${FASTA_FILES[$i]}" > "${fasta_outfile_renamed}"
done
gzip -k "${FASTA_FILES[@]}"
